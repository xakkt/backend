...
exports.allWishlistProducts = async (req, res) => {
	try {
		...
		let wishlist = await Wishlist.find({ _user: res.locals.userid, _store: req.body._store }).populate('_product', 'name image').lean();
		...
		wishlist = await Promise.all(wishlist.map(async (list) => {
			...
			var wishList = await _global.wishList(res.locals.userid, req.body._store)
			var shoppingList = await _global.shoppingList(res.locals.userid, req.body._store)
			var cartProducts = await _global.cartProducts(res.locals.userid, req.body._store)
