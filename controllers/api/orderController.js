...
exports.listCartProduct = async (req, res) => { 
    ...
    try{
        ...
        var products = await Order.find({_user:cartInfo._user,_store:cartInfo._store}).lean();
...
exports.creatOrder = async (req, res) => { 
    ...
    const errors = await validationResult(req);
                ...
                var product = await Order.findOne({_user:cartInfo._user,_store:cartInfo._store,cart:{$elemMatch: {_product:cartInfo.cart._product}}});
                if(product?.cart){
                    
                   /* product =  await Order.findOneAndUpdate({_user:cartInfo._user,_store:cartInfo._store,cart:{$elemMatch: {_product:cartInfo.cart._product}}},{$set : { 
                   ...
                   */
                   ...
                   return res.json({status: "false", message: "Product is already in the cart"})
                    ...
                    product = await Order.findOne({_user:cartInfo._user,_store:cartInfo._store}); 
                    if(!product) {
                            product = await Order.create(cartInfo);
                            console.log(product)
                        }else{
...
exports.removeProductFromCart = async(req,res)=>{
    ...
    try{            
	    ...
	    var product = await Order.findOneAndUpdate({_user:cartInfo._user,_store:cartInfo._store},{$pull:{cart: { '_product': cartInfo._product }}},{new: true});
                if(product?.cart){
...
exports.updateProductQuantity = async(req,res) => {
        ...
        try{            
          ...
          //var cartProduct = await Order.aggregate([{ $unwind: '$cart'},{$match:{_user:mongoose.Types.ObjectId(cartInfo._user),_store:mongoose.Types.ObjectId(cartInfo._store),"cart._product":mongoose.Types.ObjectId(cartInfo._product)} }])
           ...
           var product =  await Order.findOneAndUpdate({_user:cartInfo._user,_store:cartInfo._store,cart:{$elemMatch: {_product:cartInfo._product}}},{$set : { 
